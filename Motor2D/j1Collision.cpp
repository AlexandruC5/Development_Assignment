...
float j1Collision::DistanceToRightCollider(Collider* coll) const
{
	...
	for (uint i = 0; i < max_colliders; i++) 
	{
		if (colliders[i] != nullptr && colliders[i] != coll && colliders[i]->type == COLLIDER_PLATFORM) //check for valid collider
		{
...
float j1Collision::DistanceToLeftCollider(Collider* coll) const
{
	...
	for (uint i = 0; i < max_colliders; i++)
	{
		if (colliders[i] != nullptr && colliders[i] != coll && colliders[i]->type == COLLIDER_PLATFORM)
		{
